...
assert_file_exists /data/config/app/inline.conf
assert_file_contains /data/config/app/inline.conf "test=true"
assert_file_perms /data/config/app/inline.conf "640"

# ─── Templates ───────────────────────────────────────────────────────────────

echo ""
echo "=== Verifying templates configuration ==="

assert_file_exists /data/config/app/rendered.conf
assert_file_contains /data/config/app/rendered.conf "app_name=bootconf-e2e"
assert_file_contains /data/config/app/rendered.conf "env=testing"
assert_file_perms /data/config/app/rendered.conf "640"
...
assert_file_contains /etc/systemd/system/bootconf-e2e-task.service "After=multi-user.target"
assert_file_contains /etc/systemd/system/bootconf-e2e-task.service "Type=oneshot"
assert_file_contains /etc/systemd/system/bootconf-e2e-task.service "WantedBy=multi-user.target"
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 12: SHELL FIRSTBOOT — SECOND RUN SKIPS COMMAND
# ═══════════════════════════════════════════════════════════════════════════════
...
if [ "$BEFORE_MTIME" = "$AFTER_MTIME" ]; then
    PASS=$((PASS + 1))
    echo "  PASS: first-boot-only log not rewritten on second run"
else
    FAIL=$((FAIL + 1))
    echo "  FAIL: first-boot-only log was rewritten — firstboot sentinel not working"
fi
